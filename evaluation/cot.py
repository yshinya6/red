...
def eval_model(args):
    ...
    for line in tqdm(questions, total=len(questions)):
        ...
        # [Step 1] Rationale Generation (or loading from input)
        query_rationale = f"Question: {question.replace('\n', ' ')}\n\n" + rationale_inst
        ...
        query_answer = header + rationale + "\n\n" + answer_inst + "\nQuestion: " + question
        model_kwargs = make_input(processor, image_path, query_answer).to(model.device, dtype=torch.bfloat16)
        input_len = model_kwargs["input_ids"].shape[-1]
